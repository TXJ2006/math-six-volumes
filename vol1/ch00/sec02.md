...
> | 层 | 量词 | 含义 | 由谁控制 |
> |:---:|:---:|------|------|
> | 第一层 | $\forall \varepsilon > 0$ | 对手挑选任意精度要求 | 外部（对手/挑战者） |
> | 第二层 | $\exists N \in \mathbb{N}$ | 我方找到一个足够大的起点 | 证明者（可依赖 $\varepsilon$） |
...
| 层 | 量词 | 角色 | 依赖关系 |
|:---:|:---:|------|------|
| 1 | $\forall \varepsilon > 0$ | 任意输出精度 | 无依赖 |
| 2 | $\exists \delta > 0$ | 存在输入范围 | $\delta$ 可依赖 $\varepsilon$ 和 $x_0$ |
| 3 | $\forall x \in D$ | 范围内所有点 | 无依赖 |
...
> | 量词表达 | 集合论表达 |
> |:---|:---|
> | $\forall x \in D,\; P(x)$ | $\{x \in D : P(x)\} = D$ |
> | $\exists x \in D,\; P(x)$ | $\{x \in D : P(x)\} \neq \varnothing$ |
> | $\neg\exists x \in D,\; P(x)$ | $\{x \in D : P(x)\} = \varnothing$ |
