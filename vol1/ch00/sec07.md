...
但要把这两个概念真正理解透，需要超出教材的标准叙述。Boole 在 1854 年的 *The Laws of Thought* 中最早把逻辑运算（$\lor, \land$）与类的运算（并、交）系统对应起来，建立了"逻辑就是集合代数"的基本图景。Cantor 在创立集合论时虽然更关注无穷集的比较，但他处理"Vereinigung"（并）和"Durchschnitt"（交）的方式已经蕴含了现代定义的核心——通过成员资格条件来组合集合。Dedekind 在 *Was sind und was sollen die Zahlen?* 中独立引入了"Gemeinheit"（公共部分，即交）的概念。Peano 在 1888-1895 年间发明了后来成为标准的 $\cup$ 和 $\cap$ 符号——这不是偶然的记号选择，而是为了在形式语言层面精确对应逻辑联结词。Hausdorff 在 *Grundzüge der Mengenlehre*（1914）中则首次系统整理了并交的代数性质（交换律、结合律、分配律、吸收律），把它们组织成一个完整的运算体系。Zermelo 在 1908 年的公理化集合论中，特别把**并集公理（Axiom of Union）**列为独立公理——这意味着"取并集"不是可以从其他公理推导出来的操作，它需要自己的存在性保证。Peirce 在 19 世纪末的逻辑代数工作中则从另一个角度——**关系的组合**——来理解并与交，为后来的关系数据库理论和 Codd 的关系模型埋下了伏笔。
...
这正是 Velleman 一类教材反复强调的**结构对应**：集合语言与命题逻辑并不是两套平行体系，而是同一结构的两种表达。Boole 在 *The Laws of Thought* 中通过代数记号把这种对应首次形式化，Schröder 在 *Vorlesungen über die Algebra der Logik*（1890-1905）中将其发展为系统的**关系代数**。Stone 的表示定理（1936）最终证明了这种对应不是巧合：**每个布尔代数都同构于某个集合域上的集合运算代数**。Gentzen 在 1935 年建立的**自然演绎系统**则从证明论的角度揭示了这个对应的结构基础：$\land$ 的**引入规则**（从 $P$ 和 $Q$ 得到 $P \land Q$）恰好对应交集证明中的"同时验证两个条件"；$\lor$ 的**消去规则**（从 $P \lor Q$ 出发做情形分析）恰好对应并集证明中的"分情况讨论"。这不是记忆术，而是结构同构。
...
>
> **($B \cup A \subseteq A \cup B$)** 任取 $x \in B \cup A$，则 $x \in B$ 或 $x \in A$，即 $x \in A$ 或 $x \in B$，故 $x \in A \cup B$。
>
...
> $$
>
> 中间一步用的是逻辑合取的交换律 $P \land Q \equiv Q \land P$。$\square$

这两个证明虽然短，但它们体现了集合相等的核心方法论：不是看式子长得对称，而是看元素资格在两个方向上是否一致。交换律之所以成立，根本原因是逻辑上
...
> $$
>
> 中间一步用的是逻辑析取的结合律 $(P \lor Q) \lor R \equiv P \lor (Q \lor R)$。$\square$
...
不必纠结先并哪两个，最终结果一致。这不是书写方便的小事——Birkhoff 在 *Lattice Theory* 中指出，结合律是使二元运算能够自然扩展为 $n$ 元运算的必要条件。Grätzer 在 *General Lattice Theory* 中进一步阐明：没有结合律，$A_1 \cup A_2 \cup A_3$ 就不是一个良定义的表达式——它会依赖加括号的方式。对本科生来说，这一点初看似乎只是书写方便，但事实上它在告诉你：并与交具有真正的代数结构，它们不是临时发明的图形操作，而是稳定的二元运算。
...
>
> - 若 $x \in A$，则 $x \in A \cup B$ 且 $x \in A \cup C$（因为 $A \subseteq A \cup B$ 和 $A \subseteq A \cup C$），故 $x \in (A \cup B) \cap (A \cup C)$。
...
>
> - 若 $x \in A$，则 $x \in A \cup (B \cap C)$，结论成立。
...
这两条分配律值得与数的运算做一个对比。在实数算术中，乘法对加法满足分配律（$a \cdot (b + c) = ab + ac$），但加法对乘法**不**满足分配律（$a + bc \neq (a+b)(a+c)$ 一般而言）。然而在集合论中，**并对交和交对并都满足分配律**——这是一个完美的对称性。Birkhoff 在格论中把这种双向分配律称为**分配格（distributive lattice）**的标志性性质。幂集 $\mathcal{P}(X)$ 关于 $\cup, \cap$ 构成分配格，而一般的格不一定满足分配律——这说明集合运算的代数结构比许多抽象代数结构更"好"。Dedekind 最早在 1900 年注意到模律（modular law）与分配律的区别，并构造了最小的非分配格（即后来所谓的 $N_5$ 和 $M_3$）。Grätzer 在 *General Lattice Theory* 中详细讨论了这些反例，指出分配律的两条并不独立——在格的语境中，其中任一条蕴含另一条，这是 Birkhoff 和 Ward 于 1933 年证明的。
...
> **证明.** 并集：$x \in A \cup A \iff x \in A \lor x \in A \iff x \in A$，由析取幂等律 $P \lor P \equiv P$。
>
> 交集：$x \in A \cap A \iff x \in A \land x \in A \iff x \in A$，由合取幂等律 $P \land P \equiv P$。$\square$
...
> **证明（第二条）.**
>
> **($\subseteq$)** 任取 $x \in A \cap (A \cup B)$。则 $x \in A$ 且 $x \in A \cup B$。由第一个条件 $x \in A$，结论已成立。
>
> **($\supseteq$)** 任取 $x \in A$。由 $x \in A$ 得 $x \in A \cup B$（因 $A \subseteq A \cup B$）。因此 $x \in A$ 且 $x \in A \cup B$，即 $x \in A \cap (A \cup B)$。
>
...
吸收律在代数中有独特地位。Whitehead 和 Huntington 在 20 世纪初研究布尔代数和格的公理系统时，发现**吸收律是区分格与环的关键**——环满足分配律但不满足吸收律，格满足吸收律但其加法不满足消去律。Dedekind 最早在 1900 年的论文中研究了满足吸收律的代数结构（他称之为"Dualgruppe"），这正是后来格论的萌芽。De Morgan 在 1847 年的 *Formal Logic* 中已经注意到逻辑中类似吸收律的化简规则，虽然他没有使用现代的代数语言。值得一提的是，吸收律也在 Heyting 代数（直觉主义逻辑的代数模型）中成立——Heyting 在 1930 年证明，即使放弃排中律，并与交仍然满足吸收律。这说明吸收律比补运算更"基础"。
...
> **命题（单调性）.** 对任意集合 $A, B, C$，若 $A \subseteq B$，则
>
> (d) $A \cup C \subseteq B \cup C$；
>
> (e) $A \cap C \subseteq B \cap C$。

> **证明.**
>
> (d) 任取 $x \in A \cup C$，则 $x \in A$ 或 $x \in C$。若 $x \in A$，由 $A \subseteq B$ 得 $x \in B$，故 $x \in B \cup C$。若 $x \in C$，则直接 $x \in B \cup C$。
>
> (e) 任取 $x \in A \cap C$，则 $x \in A$ 且 $x \in C$。由 $A \subseteq B$ 得 $x \in B$，故 $x \in B$ 且 $x \in C$，即 $x \in B \cap C$。$\square$

单调性说明并与交都是关于子集关系的**保序运算**：输入变大，输出也变大（至少不缩小）。Tarski 在研究不动点定理时高度依赖这一性质——他在 1955 年证明的格上不动点定理（Knaster–Tarski 定理）的核心假设正是单调性。在实分析中，单调性也是理解测度论中 $\sigma$-代数封闭性的关键：若 $A_n \subseteq A_{n+1}$，则 $\bigcup_{k=1}^{n} A_k = A_n$，单调递增序列的并等于最后一个——这是 Lebesgue 测度连续性的组合前提。
...
> **命题（单位元与零元）.** 对任意集合 $A$（在全集 $U$ 中），
>
> (i) $A \cup \varnothing = A$；(ii) $A \cap \varnothing = \varnothing$；(iii) $A \cap U = A$；(iv) $A \cup U = U$。

> **证明.**
>
> (i) $(\supseteq)$ $A \subseteq A \cup \varnothing$ 显然。$(\subseteq)$ 任取 $x \in A \cup \varnothing$，则 $x \in A$ 或 $x \in \varnothing$。但 $x \in \varnothing$ 为假，故必有 $x \in A$。因此 $A \cup \varnothing = A$。
>
> (ii) 已在例 10 中证明：$x \in A \land x \in \varnothing$ 永假，故 $A \cap \varnothing = \varnothing$。
>
> (iii) $(\subseteq)$ $A \cap U \subseteq A$ 由交集定义直接得出。$(\supseteq)$ 任取 $x \in A$，因 $A \subseteq U$，故 $x \in U$，从而 $x \in A \land x \in U$，即 $x \in A \cap U$。因此 $A \cap U = A$。
>
> (iv) $(\supseteq)$ 任取 $x \in U$，则 $x \in A \cup U$（因 $U \subseteq A \cup U$）。$(\subseteq)$ $A \cup U \subseteq U$ 因为 $A \subseteq U$ 且 $U \subseteq U$。因此 $A \cup U = U$。$\square$

从代数结构的角度看，$\varnothing$ 在 $(\mathcal{P}(U), \cup)$ 中扮演单位元，$U$ 在 $(\mathcal{P}(U), \cap)$ 中扮演单位元。Zermelo 在 1908 年给出集合论的公理系统时，专门设立了**空集公理**来保证 $\varnothing$ 的存在，又设立了**并集公理**来保证并运算可以执行。这些看似显而易见的性质，在公理化框架中需要明确的存在性保证。Fraenkel 和 Skolem 随后在完善 ZF 公理系统时进一步明确了这些基础设施的公理地位。
...
**实分析视角.** 开区间的并常常用来拼接更大的开放区域。一个关键定理是：$\mathbb{R}$ 中的每个开集都可以写成至多可数个不相交开区间的并（Lindelöf 在 1903 年的紧致性定理为这个结论提供了关键工具）。闭区间的交常常用来逐步压缩范围——区间套定理（Cantor 交定理）正是利用嵌套闭区间的交来定位极限点。Lebesgue 在构建测度论时，把"可测集"定义为一个关于并、交、补封闭的 $\sigma$-代数——有限并推广为可数并，这一步骤看似微小，却让测度理论的适用范围从有限集合一跃进入无穷分析。

**概率论视角.** 事件的并对应"至少一个事件发生"，交对应"所有事件同时发生"。Kolmogorov 在 1933 年的 *Grundbegriffe der Wahrscheinlichkeitsrechnung* 中把概率论公理化，其中**第三公理**（可数可加性）本质上是关于不相交并的加法规则。容斥原理（inclusion-exclusion principle）

$$
P(A \cup B) = P(A) + P(B) - P(A \cap B)
$$

就是在定量修正并集运算中"重复计算"的部分。Rényi 在 1970 年代进一步发展了条件概率空间理论，其中集合的交对应条件化操作——$P(A \mid B) = P(A \cap B) / P(B)$ 恰好是交集关系的概率版本。

**拓扑学视角.** 拓扑空间的公理要求开集族对**有限交**和**任意并**封闭——注意这里的不对称性：交只要求有限，并可以任意。这个不对称性不是人为规定，而是出于深层的数学必要性。Hausdorff 在建立拓扑学基础时清楚地意识到：无限个开集的交不一定是开集（例如 $\bigcap_{n=1}^{\infty}(-1/n, 1/n) = \{0\}$ 是单点集，不是开集），但无限个开集的并一定是开集。Kuratowski 在 1922 年给出了拓扑的等价定义——通过**闭包算子**的四条公理来刻画拓扑结构，其中闭包保并（$\overline{A \cup B} = \overline{A} \cup \overline{B}$）正是并运算与拓扑结构交汇的关键性质。Fréchet 在 1906 年引入抽象度量空间时，开集的并与交正是他分析极限性质的基本语言。

**代数视角.** 子结构（子群、子环、子模、子空间）的交通常仍保留结构，而并则往往不保留。例如两个子群的交仍是子群，但两个子群的并一般不是子群。这说明交运算在代数结构中更"安全"——它保持封闭性。这正是为什么在代数中，生成子群、生成子空间的定义都用"包含某集合的所有子结构的**交**"来给出。Emmy Noether 的抽象代数方法论正是在这一背景下展开的。Jacobson 在 *Basic Algebra* 中系统讨论了这一现象——子群格、子模格、理想格都是通过交运算获得下确界的。van der Waerden 的 *Moderne Algebra* 则最早把这种方法论写入教科书，深刻影响了 20 世纪代数学的走向。

**范畴论视角.** Mac Lane 在 *Categories for the Working Mathematician* 中把并与交推广为**余积（coproduct）** 与**积（product）** 的特殊情形。在集合范畴 $\mathbf{Set}$ 中，积是笛卡尔积，余积是不相交并。但在偏序范畴中，$\cup$ 就是余积，$\cap$ 就是积。Lawvere 在 1960 年代把集合论本身用范畴论语言重新表述，其中并与交获得了**极限（limit）**与**余极限（colimit）**的统一解释。这种统一视角解释了为什么并和交在完全不同的数学语境中反复出现——它们不是集合论的专有物，而是满足一个共同的范畴论性质（万有性质）的构造。

**复分析视角.** 复变函数论中的区域（domain）是复平面上的连通开集。两个区域的并不一定是区域（可能失去连通性），但两个相交区域的并是区域——这正是解析延拓（analytic continuation）的拓扑基础。Riemann 在 1851 年的博士论文中本质上使用了这一思想：沿不同路径延拓一个全纯函数，相当于沿不同区域做并集，而 Riemann 面的构造就是让这些并集在全局上保持一致性。Weierstrass 的解析延拓方法则更明确地依赖幂级数收敛域的并。
...
从公理化方法论的角度看，这些律则不是一堆需要记忆的公式，而是一个代数结构的**公理骨架**。Huntington 在 1904 年给出了布尔代数的第一组独立公理系统，其中核心就是交换律、分配律和补余律。Birkhoff 随后证明，仅用吸收律就可以公理化格结构。Tarski 在 1930 年代的元数学研究中发现，这些公理之间存在精密的独立性关系——去掉任何一条，都能构造出满足其余公理但不满足该条的反例。Davey 和 Priestley 在 *Introduction to Lattices and Order* 中把这些独立性结论整理成了系统的教学框架，是当代本科生学习格论的标准参考。这意味着上面列出的性质有着**内在的逻辑层次**——有些是更基本的，有些是从更基本的推出的。

值得注意的是，Lindenbaum 和 Tarski 证明了一个深刻的对应：命题逻辑中的等价类关于 $\lor, \land$ 构成的**Lindenbaum–Tarski 代数**恰好就是一个布尔代数。这从逻辑的角度证实了为什么集合运算的代数性质与命题逻辑的性质如此完美地吻合——它们本质上是同一个代数结构的两个不同实现。
...
4. 证明**中位恒等式（median identity）**：对任意集合 $A, B, C$，
   $(A \cup B) \cap (A \cup C) \cap (B \cup C) = (A \cap B) \cup (A \cap C) \cup (B \cap C)$。
   （提示：对左边反复使用分配律。）
5. 证明：若 $A \cup B = U$ 且 $A \cap B = \varnothing$，则 $B = \{x \in U : x \notin A\}$。
6. 证明：对任意集合 $A, B, C$，$A \cap (B \cup C) \subseteq (A \cap B) \cup C$。
